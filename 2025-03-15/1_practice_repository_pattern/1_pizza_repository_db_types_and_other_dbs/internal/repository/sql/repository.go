...
import (
	...
	"repository_example/internal/repository/sql/postgres"
	"repository_example/internal/repository/sql/sqlite"
)
...
func InitRepository(ctx context.Context, cfg *config.Repo) (*Repo, error) {
	slog.Debug("initializing sql repository")
	var repo repoImpl
	var err error

	switch cfg.DB {
	case "postgres":
		repo, err = postgres.NewPizzaRepository(ctx, cfg.DBConn)
	case "sqlite":
		repo, err = sqlite.NewPizzaRepository(cfg.DBConn)
	default:
